...
interface VERC1155 {
    ...
    //going to follow opensea for URI
    //there is a default uri for all token types that follows the client substitution scheme for {id}
    //any token type can define a custom uri
    //when you call uri(id) - we check if there is a custom one and return it if we find one, if not return the default.
    //when you call setCustomURI we emit the uri event.
    //https://github.com/ProjectOpenSea/opensea-creatures/blob/165d4fe6a90532a7a913b76ef33bfbfc9624c878/contracts/ERC1155Tradable.sol#L4
...
contract ERC1155Test is DSTestPlus, ERC1155TokenReceiver {
    ...
    function testBurn() public {
        ...
        //        token2.mint(address(0xBEEF), 1337, 100, "");

        //   token2.burn(address(0xBEEF), 1337, 70);

        assertEq(token.balanceOf(address(0xBEEF), 1337), 30);

        //      assertEq(token2.balanceOf(address(0xBEEF), 1337), 30);
    }
    ...
    function testFailBalanceOfBatchWithArrayMismatch() public {
        address[] memory tos = new address[](5);
        tos[0] = address(0xBEEF);
        tos[1] = address(0xCAFE);
        tos[2] = address(0xFACE);
    ...
    function testDefaultURI() public {
        token.setURI("http://test.com/{id}");
        string memory uri = token.uri(4);
        assertEq(uri, "http://test.com/{id}");
    }

    function testLongURI() public {
        string memory defaultURI = "http://test.com/1234567890123456789012345678901234567890{id}";
        token.setURI(defaultURI);
        string memory uri = token.uri(4);
        assertEq(uri, defaultURI);
    }

    function testCustomURI() public {
        string memory defaultURI = "http://test.com/{id}";
        token.setURI(defaultURI);
        token.setCustomURI(4, "http://testc.com/4");
        string memory uri = token.uri(4);
        assertEq(uri, "http://testc.com/4");

        string memory urid = token.uri(5);
        assertEq(urid, defaultURI);
    }

    function testLongCustomURI() public {
        string memory defaultURI = "http://test.com/1234567890123456789012345678901234567890{id}";
        string memory customU = "http://test.com/1234567890123456789012345678901234567890__4";
        token.setURI(defaultURI);
        token.setCustomURI(4, customU);
        string memory uri = token.uri(4);
        assertEq(uri, customU);

        string memory urid = token.uri(5);
        assertEq(urid, defaultURI);
    }

    function testReallyLongCustomURI() public {
        string memory defaultURI =
            "http://test.com/123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890{id}";
        string memory customU =
            "http://test.com/12345678901234567890123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890__4";
        token.setURI(defaultURI);
        token.setCustomURI(4, customU);
        string memory uri = token.uri(4);
        assertEq(uri, customU);
