...
from pytypes.src.Recipient import (
    ERC1155Recipient,
    NonERC1155Recipient,
...
class ERC1155Py:
    ...
    def setApprovalForAll(self, from_: Address, operator: Address, approved: bool):
        ...
        return st.TX(
            [
                ERC1155Impl.ApprovalForAll(
                    account=from_, operator=operator, approved=approved
                )
            ]
        )
...
class ERC1155FuzzTest(FuzzTest):
    ...
    def pre_sequence(self) -> None:
        ...
        self.receivers.set(
            [ERC1155Recipient.deploy() for i in range(0, random_int(3, 5))]
        )
    ...
    @flow()
    @st.given(
        to_=st.choose(addresses),
        id=st.choose(token_ids),
        amount=st.random_int(max=200),
        data=b"",
    )
    def flow_MintToEOA(self, **kwargs) -> None:
        ...
        self.invoke(
            "mint",
            expected_execptions=[IERC1155Errors.ERC1155InvalidReceiver, Panic],
            **kwargs
        )

    @flow()
    @st.given(
        to_=st.choose(receivers),
        id=st.choose(token_ids),
        amount=st.random_int(min=0, max=200, min_prob=0.04),
        data=st.random_bytes(min=5, max=20),
    )
    ...
    @flow()
    @st.given(
        from__=st.choose(addresses),
        id=st.choose(token_ids),
        amount=st.random_int(min=0, max=200, edge_values_prob=0.05),
    )
    def flow_Burn(self, **kwargs) -> None:
        self.invoke(
            "burn",
            expected_execptions=[IERC1155Errors.ERC1155InsufficientBalance],
            **kwargs
        )

    @flow()
    @st.given(
        from_=st.choose(addresses),
        operator=st.choose(addresses),
        approved=st.random_bool(true_prob=0.3),
    )
    def flow_changeApproval(self, **kwargs):

        self.invoke(
            "setApprovalForAll",
            expected_execptions=[IERC1155Errors.ERC1155InvalidOperator],
            **kwargs
        )
    ...
    def invariant_approvals(self) -> None:
        for from_, approvals in self.erc1155py.approvalMap.items():
            for operator in approvals.keys():
                try:
                    self.erc1155.isApprovedForAll(
                        from_, operator
                    ) == self.erc1155py.isApprovedForAll(from_, operator)
                except:
    ...
    def invariant_balances(self) -> None:
        for addr, balances in self.erc1155py.balanceMap.items():
            for id in balances.keys():
                try:
                    self.erc1155.balanceOf(addr, id) == self.erc1155py.balanceOf(
                        addr, id
                    )
                except:
