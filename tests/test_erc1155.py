...
from pytypes.helpers.Recipient import (
    ERC1155Recipient,
    NonERC1155Recipient,
...
def mint(operator, to, id, count, data):
    ...
    assert tx.events == [
        ERC1155Impl.TransferSingle(operator, Address(0), to, id, count)
    ]
...
def test_MintToEOAOverflow():
    ...
    assert tx.events == [
        ERC1155Impl.TransferSingle(tx.from_.address, Address(0), to, id, MAX_UINT)
    ]
...
def test_MintToEOABatch():
    ...
    assert tx.events == [
        ERC1155Impl.TransferBatch(operator, Address(0), to, ids, counts)
    ]
...
def test_FailBatchMInt():
    ...
    with must_revert(
        IERC1155Errors.ERC1155InvalidArrayLength(len(ids), len(counts))
    ) as e:
...
def test_MintToERC1155Recipient():
    ...
    assert tx.events == [
        ERC1155Impl.TransferSingle(operator, Address(0), to.address, id, count)
    ]
...
def transferFrom(from_, to, mint_count, transfer_count):
    ...
    assert tx.events == [
        ERC1155Impl.TransferSingle(operator, from_, to, id, transfer_count)
    ]
...
def test_FailSafeTransferFromSelfInsufficientBalance():
    ...
    to = Address("0x00000000000000000000000000000000000000bb")
    with must_revert(
        IERC1155Errors.ERC1155InsufficientBalance(
            sender=from_, balance=100, needed=101, tokenId=1432
        )
    ) as e:
